...
for node, ip in dev_dic.items():
   ...
   dev.open()
   ping_mgmt_gw = dev.rpc.ping(host="10.117.50.1", count="10", rapid=True)
   if ( ping_mgmt_gw.find('.//ping-success') is not None):
      print ("Destination %s is Reachable!" % "10.117.97.1")
   else:
      print ("Destination %s is NOT Reachable!" % "10.117.97.1")
